...
const returnLocalhostUrl = 'http://127.0.0.1/transbank-response';
const returnAzureUrl =
    'https://eastus2.azure.data.mongodb-api.com/app/geocarapp-sduybtn/endpoint/commit';
    ...
    try {
      ...
      const transaction = await ( new transbank.Transaction()).create(
        ...
        amount = req.body.amount,
         return_url = returnLocalhostUrl,
      );

        // Extract token and URL from the transaction response
        // token is the same along transaction
...
router.get('/transbank-response', async (req, res) => {
  try {
      ...
      const commitResponse = await ( new transbank.Transaction()).commit(token_ws).then();

    // Verificar que response_code sea igual a cero
    if (commitResponse.response_code === 0) {
       // Renderizar la plantilla HTML con la respuesta de Transbank

        // Extract values from transaction response
        const {
          vci,
          amount,
          status,
